...
!=======FEW THINGS: 
! MAYBE MAKE c(k) a function of zenith angle?????????????
!!1) LANDSNOWFRAC = 1 for coldstart + FRACDO = 1 
...
integer, parameter :: nbelts = 36 ! 36 latitudinal belts.. 87.5
!integer, parameter :: nbelts = 60 ! 60 latitudinal belts.. 88.5
!integer, parameter :: nbelts = 72 ! 72 latitudinal belts.. 88.75
!integer, parameter :: nbelts = 90 ! 90 latitudinal belts.. 89.0
!integer, parameter :: nbelts = 180 ! 180 latitudinal belts.. 89.5.. I DO get a lot more CO2 condensation here, and the right trends, but the rest of the planet gets warmer! Maybe d too high at too high resolution?
...
albave, tempave,  fluxavesum, albavesum, tempavesum,  wthratesum, &
 ...
 fn2, cptot, gamd, co2sicesum, co2siceave, co2siceavesum, ann_co2siceave, &
  icealbco2, fracd0, sumd0  !c-rr added fracd0, sumd0, and liql 6/16/2019 

real*4 :: Ls, Lssteps, latradsun, Lso, delpress, hcpn2, sumpco2
real*4 subl(nbelts), liql(nbelts), CO2ps(nbelts), sumCO2sice, ell, ccloud, delpe, TCOND
...
fwthr, pco2i, smass, mstar, newice, pressi, pressf
...
!if(tempi.ge.273)fracd0(1:nbelts) = 1.d0
...
! OLD outgassing rates and corrections for old weathering parameterization 
!v =   3.3e14  !volcanic outgassing for Earth (g/year) [from Holland (1978)].. should be scaled  for different planets
! conv = 2.5727 ! constant needed so that at T = 288 K, Earth gets weathering rate of 3.3e14 g/yr
!wco2 = conv*v !carbonate-silicate weathering rate (~ 8.5e14 g/yr) for Earth
...
hcpn2 = 0.2484     ! heat capacity of n2 (cal/g K)
...
!	  h20alb(90) = 0.44   ! ocean albedo at 90 degrees incidence with briegleb parameterization	 
!      do  i = 1,90,1
!        write(60,*)h20alb(i)
!      enddo
...
midlatrad(1) = -87.5*pi/180. ! 36 belts
midlat(1) = -87.5  ! 36 belts

!midlatrad(1) = -88.75*pi/180.   ! 72 belts
!midlat(1) = -88.75   ! 72 belts

!midlatrad(1) = -88.5*pi/180.   ! 90 belts
!midlat(1) = -88.5   ! 90 belts
...
!Pnc = Pdry + pco2i  ! pressure of non-condensible phase (bar)
 
!fnc = Pnc/Pot  ! non-condensible mixing ratio, (N2 + CO2)


 ! actual CO2 mixing ratio with respect to dry phase, allowing CO2 to diffuse with height.. So pco2 = 5.19e-4 bar becomes 3.3e-4 bar (330 ppm)


!fco2(1:nbelts) = fco2(1:nbelts)*fnc ! actual CO2 mixing ratio. 


!pco2(1:nbelts) = fco2(1:nbelts)*Ptot    ! the real PCO2 wrt all air(in bar)
...
!	write(60,*)t, last, orbitcount
!    write(70,*)tcalc, compcnvg, orbitcount, last
...
!              if(nstep.eq.0)Ls = 0.
!===================================================
!              Ls = Ls + 8.891484862e-4  ! solar longitude..From Armstrong et al. (2014)
!               latradsun = oblrad*sin(Ls)
!              print *, Ls
!====================================================
               Ls = asin(dec/oblrad)


!              print *, Ls, nstep, dec*pi/180.
!              
...
!       fco2(k) = pco2(k)/Ptot  
       fco2(k) = pco2(k)/psurf(k)   ! Changed from Ptot to psurf(k), which includes pH2O 
              ...
              if (fco2(k) .lt. 1.e-1)then  ! Changed from .le. to .lt. c-rr 11/13/2019
...
!		  write(40,*)TL, TR, PL, PR, temp(k), k
	  ...
	  ! X0 = tempgrid(TL)        ! Y0 = PLOGL
	  ! X1 = tempgrid(TR)        ! Y1 = PLOGR
	  ! X2 = temp(k)             ! Y2 = pco2rlog
...
!	  if ((TL.eq.1).and.(PR.eq.1).and.(TR.eq.1).and.(PL.eq.1))Z4 = olr(1,1)	   
...
!  DIURNALLY-AVERAGED ZENITH ANGLE                            
      ...
      if ((tan(dec)*tan(asin(x(k)))).ge.1.) then 
         ...
         h = acos(-x(k)*tan(dec)*((1-x(k)**2)**(-0.5)))
           !      h = acos(-tan(latrad(k))*tan(dec))
         mu(k) = x(k)*sin(dec) + cos(asin(x(k)))*cos(dec)*sin(h)/h
      end if
           

! From Armstrong et al. (2014)
!        if((latrad(k).le.(-pi/2 + latradsun)).or.(latrad(k).le.(pi/2+latradsun)))then
!         h = pi
!         mu(k) = x(k)*sin(latradsun) + cos(asin(x(k)))*cos(latradsun)*sin(h)/h
!        elseif ((latrad(k).ge.(pi/2 - latradsun)).or.((latrad(k).le.(-pi/2 - latradsun))))then      
!          h = 0.
!          mu(k) = 0.
!        else
!         h = acos(-tan(latrad(k))*tan(latradsun))
!         mu(k) = x(k)*sin(latradsun) + cos(asin(x(k)))*cos(latradsun)*sin(h)/h
!        end if


!        if((latrad(k).le.(-pi/2 + dec)).or.(latrad(k).le.(pi/2+dec)))then
!         h = pi
!         mu(k) = x(k)*sin(dec) + cos(asin(x(k)))*cos(dec)*sin(h)/h
!        elseif ((latrad(k).ge.(pi/2 - dec)).or.((latrad(k).le.(-pi/2 - dec))))then      
!          h = 0.
!          mu(k) = 0.
!        else
!         h = acos(-tan(latrad(k))*tan(dec))
...
!           if (dec.ge.10*pi/180.)then
!            if(last.eq.1)write(40,*) h, mu(k), dec*180./pi, Ls*180./pi
!           pause
!           endif
...
!      if(zrad.ge.1.54)then  ! The thing is that zrad is getting bigger at high obliquity than low obliquity.. maybe messing things up? c-rr 8/5/2019
!      zrad = 1.54
!      mu(k) = 0.0308
!      endif
...
!aice_vis = aice_nir
  ...
  if(icealbco2(k).gt.0.1)snowalb = icealbco2(k)  ! If CO2 is condensing within latitude band, that becomes the new ice albedo for that band..Also, assumes that minimum cloud fraction is 10% (unless CO2 is condensing or climate is warm enough for H2O clouds to form)..c-rr 6/11/2019

  if(imco2i(k).eq.1)fcloud(k) = ccloud(k) ! if CO2 clouds are in the band, then flcoud(k) is that!! c-rr 11/13/2019
  if(ann_tempave.ge.263) fcloud(k) = h2ocloud(k)  ! if h2oclouds are forming instead, use h2ocloud(k)
...
!      if (temp(k).le.256.d0) then	  ! 256 K ensures that mean surface temperature below ~ 263 - 264 K, leads to total glaciation... for dense CO2 atmospheres
      ...
      ci = 1.05e7  ! thermal heat capacity of ice  (cl and  cw is the same for  land and water,  respectively), ci = 1.05e7
...
!	   if(last.eq.1)write(60,*)tstrat
!	   if(last.eq.1)write(60,*)tstrat, temp(k), pco2r(k), mu(k)
...
!
      nh = 0
!      if((k.eq.1).or.(k.eq.2).or.(k.eq.17).or.(k.eq.18))then  ! poles
!	  tlapse = 3!10.
!	  elseif ((k.ge.7).and.(k.le.12))then ! tropics
!	  tlapse = 6.5 !3.5
!      else
!      tlapse = 5!6.5 !subtropics   !(lapse rate)degrees Kelvin per kilometer.. BUT THIS SHOULD DEPEND on THE PLANET, RIGHT? 6.5K/km is  average for Earth!!!
!	  endif
...
!       if(last.eq.1)write(40,*)hpco2, psat, htemp, imco2l(k), imco2i(k) Why is imco2l flag not changing to 1?
        ...
        ccloud(k) = 0.  ! when CO2 is not condensing this is zero... This feeds in fcloud(k) too.. unless temps warm enough for h2o clouds to form.
!        imco2s(k) = 0 
!	imco2l(k) = 0
...
!=========================c-rr 8/6/2019
         if((last.eq.1).and.(pco2flag.eq.1))then  !For Irene's project: In terms of determining if CO2 condensation occured, only count the condensation that occurs after converged to last orbit c-rr 8/6/2019
          pco2i = ann_pco2ave
          pco2flag = 0
          endif
!=====================================
...
!         print *, htemp, hpco2, psat, psat*101325./g
!	pause

        ! THE AMOUNT OF CO2 CLOUD FORMED c-rr 7/27/2019
!         delp = hpco2 - psat ! in bars
!        delpe = 0.1 ! in bars, corresponds to ~50-55% cloud cover.. For 2 bar CO2 atmosphere, 1-D model showed CO2 first condenses at height where P = 1.1 bar and ~195 K. Psat at 195 K is 1 bar.. So 1.1 - 1. = 0.1 bar for 50% cloud cover
!         delp = zrad
!         delpe = 1.047 ! average zenith angle (in rads) corresponding to 60 degrees and 50 - 55% CO2 cloud cover..zenith angle (rads) is above ~1.54, 100% cloud cover
!        ccloud(k) = min(0.725*log((zrad/1.047) + 1), 1.)  !  changed from 0.8 in original param to 0.72.. 
         ccloud(k) = 0.5 ! maybe 0.5, alternatively 1?. CO2 cloud cover fraction per latititude band

         cloudir(k) = -0.0  ! For now, I have no idea how much cooling CO2 clouds do, especially as a function of latitude or obliquity (c-rr 11/14/2019)
         fcloud(k) = ccloud(k)  ! c-rr 11/13/2019... use ccloud(k) to define fcloud for each latitude band when CO2 clouds are condensing..

!   LOW HEATING EFFICIENCY + CONSTANT cloud coverage over planet with obliquity gives right obliquity trends c-rr 8/6/2019

         
! Play more games with ice heat capacity? Emissivity seems to do nothing either..
...
!	   endif ! ends last logic for CO2 clouds/ice
...
!		write(40,*)delp, pco2(k), psat, k
         ...
         pressi = pco2(k)  ! initial pressure in latitude band after ice condenses, sublimates...etc
!         if(k.eq.5)then
!         print *, pco2(k), psat, temp(k),k 
!         pause
!         endif
...
!         print *, temp(k), pco2(k), psat
!         pause

!         write(70,*)delp, pco2(k), psat, k, imco2s(1:nbelts)		
...
!        write(40,*) temp(k), delp,pco2(k), psat, k, imco2s(k)

!                write(40,*), pco2(k), psat, zicesum(k), k
             CALL WEATHERING2(area, k, pco2, nbelts, focean, fwthr, & 
	       wthrate, warea, wco2,wco2e,v,dt, temp)
            
!             write(40,*) temp(k), delp,pco2(k), psat, k, imco2s(1:nbelts)
...
!	imco2s2(k) = 1
!	     if(last.eq.1)write(40,*)imco2l(k)
...
!                if(last.eq.1)write(70,*)temp(k), TCOND, pco2(k), k
                 ...
                 subl(k) = 0.
                 newice(k) = 0.0 !c-rr 7/23/2019... no new ice formed this time around
...
!                         if(last.eq.1)write(90,*)temp(k), TCOND, pco2(k), k
...
!                 write(40,*), subl(k), zicesum(k), pco2(k), psat, temp(k), k
	     ...
	     CALL WEATHERING2(area, k, pco2, nbelts, focean, fwthr, & 
	       wthrate, warea, wco2,wco2e,v,dt, temp)
               ...
               pressf = pco2(k) ! final pressure in latitude band after ice condenses, sublimates...etc
               delpress = pressi - pressf ! change in pressure due to CO2 condensation/sublimation at this latitude band and time step
           ...
           ! If there is no surface ice accumulation in given band, then turn CO2 surface ice flag in that band to "zero." c-rr 6/6/2019
            if(imco2s(k).eq.1)then
                if(zicesum(k).le.0)then
                 imco2s(k) = 0
!                 newice(k) = 0.0 ! c-rr 7/23/2019.. come back!!!
                 
               endif
            endif

!         write(40,*) temp(k), delp,pco2(k), psat, k, imco2s(1:3)	  
!        print *, imco2s(1:nbelts)
!        pause
...
!          if(last.eq.1)write(50,*)atoa(k), k
...
!        s(k) = (q/pi)*(h*sin(latradsun)*sin(latrad(k)) + sin(h)*cos(latradsun)*cos(latrad(k))) ! From Armstrong et al. (2014)
...
!  SURFACE TEMPERATURE - SOLVE ENERGY-BALANCE EQUATION 
                ...
                ! c-rr 8/6/2019.. Apparently, colder atmospheres should have lower d0. Ask Irene to find out if there is a T dependence for this.
!               if(icoldflag.eq.1)then ! for cold starts
!               fracd0(k) = 0.15d0 ! old constant d assumption.. This agrees with Turbet et al. (2017) Figure 1.
!               else ! for warm starts
...
!                                              endif                                                        
                ...
                latefac =  (ann_tempave/288.)**alphal ! latent heat factor to diffusion equation.
                if (ann_tempave.ge.288)latefac = 1.
...
!       print *, temp(k), newice(k), c(k)

!          if(last.eq.1)then
!          write(40,*) acos(mu(k))*180./pi, k  ! zenith angles varies from ~50 to 90 degrees, apparently
!          endif
...
!	  ph2o(k) = RH_s*po_star*exp(-L/(Rgas*temp(k)))/(101325.) ! average H2O surface pressure at each latitude (in bar)
...
!=============== CALCULATING THE CONVECTIVE HEAT FLUX (FC)==============================================
          ...
          fh2o_real(k) = ph2o(k)/psurf(k) ! real FH2O
          fco2_real(k) = pco2(k)/psurf(k) ! real FCO2 
...
!          FL = 1.e-3*LL*Cd*vel*rhos*qh2o(k)*(1. - RH_s) ! Latent heat flux surface (W/m^2 or kg/s^3) , with L in J/kg

          ! Make FL independent of RH_s because you don't want more clouds at lower RH_s.. 1 - 0.77 = 0.23 .. c-rr 11/14/2019
          FL = 1.e-3*0.23*LL*Cd*vel*rhos*qh2o(k)

  ! J/g/K J/kg  m/s  kg/m^3  g/kg

!           print *, FL, LL, Cd, vel, rhos, qh2o(k), RH_s, cp, hcp
!           pause
...
!====================================================================================================
          ...
          cloudir(k) = max(-7.2*log((FC/FE)+1),-10.) ! updating H2O cloud cloudir for every latitude 
          h2ocloud(k) = fcloud(k) 
          endif
...
! SHOULD REMEMBER IF "DRY" ATMOSPHERE, THEN THERE ARE NO CLOUDS!!!!!!!!!!!!!!

!           print *, cloudir(k)
!           pause

           ! do similar scaling as fcloud(k) for cloudir(k)

! qh2o earth ~ 8.4, rhos = 1.225 , RH_s = 0.77, vel = 5, Cd = 1.5e-3, L =2.5e6, FLEARTH = 88.8, FSEARTH = 1.286
...
!  SUM FOR GLOBAL AVERAGING  (WITHIN A GIVEN TIME STEP.. SUMMING AREA CONTRIBUTION OVER ALL LATITUDES TO GET GLOBAL IR, FLUX, PALB, TEMP..etc)
      ...
      co2icesum = co2icesum + area(k)*imco2i(k)*ccloud(k) ! updated to calculated CO2 cloud fraction c-rr 7/27/2019
	  co2lsum = co2lsum + area(k)*imco2l(k)
          h2osum = h2osum + area(k)*h2ocloud(k)  ! summing over H2OCLOUD c-rr 8/8/2019
...
!	  write(60,*)  
CO2ps(k) = pco2i - pco2(k)  !  CO2 pressure on the surface at a given latitude band for a given time step.
sumCO2sice = sumCO2sice + CO2ps(k)*area(k)  ! For a given time step, the total amount of surface CO2 (in bar) for a given time step

!write(50,*)CO2ps(k), k, tcalc/3.15e7
...
write(50,*)sumCO2sice, tcalc/3.15e7
sumCO2sice = 0.       
...
!	   write(40,*)pco2avesum, pco2ave, orbitcount
!      write(40,*)globwthrate, wthratesum, orbitcount
!  if(sumsteps.eq.1)write(60,*)irave, albsum, fluxave, albave
!       write(40,*)co2iceave, co2cldave
...
!                  if(ann_tempave.ge.273.15)then  ! c-rr 7/19/2019
!                  cloudir(k) = -.4040404*ann_tempave + 110.3636353  ! cloudir = -6.0 at 288 K, almost zero at 273.15.. NOT NEEDED because I am not calculating cloudIR(K) with the water cloud parameterization..
!                 landsnowfrac = 0.5
!                  endif

!             if(ann_tempave.ge.273)then  ! let fcloud in warm atmospheres evolve up to 0.55 at 288 K (c-rr 7/1/2019) ! don't need this anymore cuz got new water cloud parameterization c-rr 8/6/2019 
!                if ((ann_tempave.ge.273.15).and.(ann_tempave.lt.288)) fcloud(k) = 0.037037037037036986*ann_tempave -10.111111111111097
!             endif 
...
!                 print *, fcloud(k)
        ...
        ! ALLOW FRACDO TO EVOLVE NATURALLY.. NO! Don't do this!!! Lots of weird energy balance issues if you do this and this is ad-hoc.
!	fracd0(1:nbelts) = (0.12 + 0.0063013699*(ann_tempave - 200.))/0.58  
...
!      print *, fcloud
	  ...
	  ann_psurfave = psurfavesum/nstep
                   ...
                   print *, ann_fracd0ave   
!	     write(40,*)orbitcount, ann_pco2ave
...
!     write(40,*)wthr, wco2e, orbitcount
	 
!   ADJUST DIFFUSION COEFFICIENT .. don't calculate this if nwthr lt. 5 or if wthr .gt. v .. no! do this after every orbit! c-rr 6/15/2019
!      if((nwthr.ge.5).and.(wthr.lt.v).and.(wthr.lt.1.e-2))then

!	  write(40,*)orbitcount, ann_pco2ave, wthr
!      pco2sum = 0.0
	  
!	  do i = 1, nbelts
!	  pco2sum = pco2sum + pco2(i)
!	  enddo
	  
!	  pco2avg = pco2sum/nbelts
	  
!      write(40,*)ann_pco2ave


!mass = 28.*Pdry/(Pdry+pco2(k)) + 44.*pco2(k)/(Pgen+pco2(k)) + 18. 
! molecular weight = 28.965 g/mol if Earth air. 28 g/mol for N2.


! COMMENTING OUT THIS LOGIC.. making d calculated at "EVERY" TIME step..
!      avemol = mp*((fn2_real*28.  + fh2(k)*2.)*Pdry + 44.*ann_pco2ave)/(Pdry+ ann_pco2ave) ! molecular weight expression.... Pdry remember is composed of H2 + N2 in this case, not just N2 c-rr 6/9/2019
!      avemol = mp*(28.*Pdry + 44.*ann_pco2ave)/(Pdry+ ann_pco2ave) ! molecular weight expression, assuming N2 as Pdry....
!      avemol = mp*(28.965*Pdry + 44.*ann_pco2ave)/(Pdry+ ann_pco2ave) ! molecular weight expression for Earth air instead.
!      hcp = (hcp0 + ann_pco2ave*hcpco2)/(Pdry+ann_pco2ave)  ! heat capacity
!      d = d0*((Pdry+ ann_pco2ave)/po)*((avemol0/avemol)**2)*(hcp/hcp0)* &   
!        (rot0/rot)**2




!      if(orbitcount.eq.33)then
!     print *, d, d0, avemol0, hcp, hcp0
!     pause	  
!      endif 


!	  write(40,*)ann_pco2ave, ann_ph2oave
!      write(40,*)ann_tempave, ann_albave, ann_fluxave, ann_wthrateave, wco2e, ann_pco2ave, ann_psurfave, d, tcalc, orbitcount	  
	  ...
	  ann_pco2ave, ann_ph2oave, ann_psurfave, wco2e,d, pco2, nbelts, modelout,zntempmin, tcalc, orbitcount)
		  ...
		  zndecmin, zndecmax,zntempmax, zntempmin, ann_co2iceave, igeog, &
		   focean,ann_co2lave, coastlat, ocean, seasonout, midlat,x, ann_pco2ave, pco2i)
             ...
             if(ann_pco2ave.lt.0.999*pco2i)print *, 'SURFACE CO2 CONDENSATION HAS OCCURRED.'  ! c-rr 8/5/2019 .. Now more than 0.1% then it says surface CO2 condensation
...
! Below a temp of 266, you can't get a solution no matter what the beta value is.. The weathering rate curves don't go down to such low temperatures.

! Remember can't go above P = 35 bar
...
!ph2o(1:nbelts) = po_star*exp(-L/(Rgas*tempi))/(101325) ! initial H2O pressure in bar

!mass = 28.*Pgen/(Pgen+pco2(k)) + 44.*pco2(k)/(Pgen+pco2(k))  ! molecular mass atmosphere.. The 28 must keep changing depending  on relative proportion of N2 and H2O


!pco2e = pco2(k)/(((Pgen+pco2(k))/mass + pco2(k)/44.)*44.)  ! converting from pco2 allowing CO2 to diffuse with height.. So pco2 = 5.19e-4 bar becomes 3.3e-4 (330 ppm)
!fco2e = pco2(k)/(((Pgen)/28. + pco2(k)/44.)*44.)


!FCO2 = PCO2/((PN2/28.+PCO2/44.)*44.) 

!write(40,*)mass, pco2e, pco2(k)
...
!if(k.eq.1)write(40,*)wthrate(k), wco2,wco2e,temp(k), pco2(k)

!       pco2(k) = pco2(k) + (v - wco2)*dt  ! Can't let P go above 35 bar

!To convert back to partial pressure EBM uses:

!pco2(k) = fco2*(44.*(Pgen/28.)+ pco2)
...
!	  write(40,*)pressgrid(1), tempgrid(1), pco2rr, tempp
! test
!k = 3
!temp(k) = 288.
!pco2r(k) = 5.e-4
!	    if(k.eq.7)write(40,*)pco2r(k), temp(k),  k

!		 if(k.eq.5)write(40,*) pco2r(k), pressgrid(P), k
...
!=====================================================================
	  ...
	  ! X0 = tempgrid(TL)        ! Y0 = PLOGL           !Z0 = zygrid(ZYL)      !T0 = salbgrid(AL)
	  ! X1 = tempgrid(TR)        ! Y1 = PLOGR           !Z1 = zygrid(ZYR)      !T1 = salbgrid(AR)
	  ! X2 = tempp               ! Y2 = pco2rlog	    !Z2 = zyy              !T2 = as	  
	   
!	   (tempp, pco2rr, as, muu, TL, PL, TR, PR,& 
!       PLOGL, PLOGR, pco2rlog)
