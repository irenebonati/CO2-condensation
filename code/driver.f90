...
DO WHILE((tcalc.le.tend))
    ...
    DO  k=1,nbelts,1   !**start belt loop 
        ...
        tprimeave(k) = (tprime(k)*delx(k) + tprime(k-1)* delx(k-1))/(delx(k) + delx(k-1))
       ...
       CALL INTERPOLR(temp(k), pco2r(k), TL, PL, TR, PR, PLOGL, PLOGR, pco2rlog)
	   Z0 = olr(TL, PR)
	   Z1 = olr(TR, PR)
	   Z2 = olr(TL, PL)
	   Z3 = olr(TR, PL)
...
! 	  end if		 
	  ...
	  N_9*K9 + N_10*K10 + N_11*K11 + N_12*K12 + N_13*K13 + N_14*K14 + N_15*K15 + N_16*K16
     ...
     if (seasonflag.eq.1) then
	   s(k) = (q/pi)*(x(k)*sin(dec)*h + cos(asin(x(k)))*cos(dec)*sin(h))		  	 
     else ! if no seasons 
    ...
    if (seasonflag.eq.1) then
        ...
        if (((abs(decmid).gt.(abs(decold))).and. (abs(decmid).gt.(abs(decnew)))).or.(decnew*decmid.le.0.))then 
 			610        format('data written to file ',a8, ' at declination ',f6.2, ' degrees')
    ...
    ! summing all global averages over ALL time steps within each orbit
	...
	!====================================================================================================================================================================
      ...
      elseif (t.ge.P)then !**one orbit since last averaging.
        ...
        !print *,pco2(34), pco2(35), pco2(36)
        !print *,temp(34), temp(35), temp(36) --> wrong
