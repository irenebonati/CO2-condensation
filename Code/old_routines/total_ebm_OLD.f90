...
!============BIG DO WHILE LOOP!!!!!!===================================================================================
	...
	if(orbitcount .eq. orbits)then
	last = 1 ! When last orbit is reached, set last flag to one.
	tlast =  0.
	endif
	
	sumsteps = sumsteps + 1

!	write(60,*)tcalc, t, tlast
...
!       if(orbitcount.eq.orbits)then  ! If in the last orbit and not converged, set prevtempave = ann_tempave. Otherwise, if converges, stop calculation early.. you are done!

!                  write(60,*)prevtempave, ann_tempave, t

!       if(abs(prevtempave-ann_tempave).lt.cnvg)then 
!        CALL WRAPUP(last,k,ann_tempave,ann_albave,ann_fluxave,ann_irave,ann_wthrateave, &
!	  wco2,d,pco2,nbelts,modelout,zntempmin, tcalc) ! only in  wrapup can last be made equal to 1 to loop over code one last time
!	  if(last.eq.1)stop ! after convergence has been achieved stop code
!       elseif (abs(prevtempave-ann_tempave).gt.cnvg)then 
!               prevtempave = ann_tempave 
!       endif  ! ends if prevtemp and wrap up call logic
          
!       endif ! end if orbitcount logic
...
!      If at the end of the orbit (about to start last) and not converged, set prevtempave = ann_tempave. Otherwise, if converges, stop calculation early.. you are done!
...
!       endif  ! ends if prevtemp and wrap up call logic
       ...
       if(last.eq.1) then  ! The last step of the last orbit (t ge. P condition)
!------------------------------------------------------------------		   
